...
class MyDatasetFunction(torch.utils.data.Dataset):
    def __init__(self, dataset_path, max_tokens=510, model_path='codegen-350M-mono', include_path=False):
        ...
        if "codet5" in model_path:
            ...
            # ------------------------------------ Tokenizer vocabulary ------------------------------------#
            self.tokenizer_vocab = AutoTokenizer.from_pretrained(model_path)
            self.tokenizer_vocab.pad_token = "<pad>"
        ...
        for each_problem in tqdm(dataset_list):
            ...
            for each_content in problem_content_list:
                if "efficient-inefficient code pair" in each_content:
                    ...
                    # Input_feature = f"NL:{total_problem_text}\nInefficient code:{slow_code}\nEfficient code:"
    ...
    def __getitem__(self, index):
        ...
        if self.include_path:
            ...
            # feature_encoding_dict = self.tokenizer_vocab(input_feature, padding=True, return_tensors="pt")
            ...
            label_encoding_dict = self.tokenizer_vocab(label_code, padding='max_length', truncation=True,
                                            max_length=self.max_tokens, return_tensors="pt")
            # label_encoding_dict = self.tokenizer_vocab(label_code, padding=True, return_tensors="pt")
...
if __name__ == '__main__':
    test_data = MyDatasetFunction(
        dataset_path="F:/ACEO-Current-Dataset-Paper1/Dataset/43-ACEO-Temporary/train",
        max_tokens=512
    )
    data = test_data[0]
    print(test_data[0])
