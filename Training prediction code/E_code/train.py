...
def 补足填充函数(一批数据):
    ...
    for 某数据 in 一批数据:
        标题.append(某数据[0])
        问题描述主体.append(某数据[1])
        Input描述.append(某数据[2])
        Output描述.append(某数据[3])
        输入输出样例测试和Note描述.append(某数据[4])
        某慢速代码.append(某数据[5])
        标签代码.append(某数据[6])

    # ---------------------------------------------------------- 转化为张量字典 --------------------------------#
    标题张量字典 = Token化Bert词表(标题, max_length=512, truncation=True, padding=True, return_tensors="pt")
    问题描述主体张量字典 = Token化Bert词表(问题描述主体, max_length=512, truncation=True, padding=True, return_tensors="pt")
    Input描述张量字典 = Token化Bert词表(Input描述, max_length=512, truncation=True, padding=True, return_tensors="pt")
    Output描述张量字典 = Token化Bert词表(Output描述, max_length=512, truncation=True, padding=True, return_tensors="pt")
    输入输出样例测试和Note描述张量字典 = Token化Bert词表(输入输出样例测试和Note描述, max_length=512, truncation=True, padding=True, return_tensors="pt")
    ...
    return [标题张量字典,问题描述主体张量字典,Input描述张量字典,Output描述张量字典,输入输出样例测试和Note描述张量字典, 某慢速代码张量字典, 标签代码张量字典]
...
def train函数(模型, 设备, 训练集数据, 测试集数据, 多块GPU, 命令行参数):
    ...
    for 某轮 in range(命令行参数.epochs):
        ...
        # ======================================================== 每结束一轮，保存模型一次 ============================#
        模型.title_layer.save_pretrained("./Generated_models/title_layer/")
        模型.问题描述主体层.save_pretrained("./Generated_models/问题描述主体层/")
        模型.Input_description_layer.save_pretrained("./Generated_models/Input_description_layer/")
        模型.Output描述层.save_pretrained("./Generated_models/Output描述层/")
        模型.输入输出样例测试和Note描述层.save_pretrained("./Generated_models/输入输出样例测试和Note描述层/")
        ...
        # ======================================================== 奇数轮备用：间隔保存，安全保障 ==================#
        if (某轮 + 1) % 2 == 1:
            模型.title_layer.save_pretrained("./奇数轮备用/title_layer/")
            模型.问题描述主体层.save_pretrained("./奇数轮备用/问题描述主体层/")
            模型.Input_description_layer.save_pretrained("./奇数轮备用/Input_description_layer/")
            模型.Output描述层.save_pretrained("./奇数轮备用/Output描述层/")
            模型.输入输出样例测试和Note描述层.save_pretrained("./奇数轮备用/输入输出样例测试和Note描述层/")
        ...
        # ======================================================== 20备用：间隔保存，安全保障 ==================#
        if (某轮 + 1)  == 20:
            模型.title_layer.save_pretrained("./第20轮保存/title_layer/")
            模型.问题描述主体层.save_pretrained("./第20轮保存/问题描述主体层/")
            模型.Input_description_layer.save_pretrained("./第20轮保存/Input_description_layer/")
            模型.Output描述层.save_pretrained("./第20轮保存/Output描述层/")
            模型.输入输出样例测试和Note描述层.save_pretrained("./第20轮保存/输入输出样例测试和Note描述层/")
        ...
        # ======================================================== 25备用：间隔保存，安全保障 ==================#
        if (某轮 + 1) == 25:
            模型.title_layer.save_pretrained("./第25轮保存/title_layer/")
            模型.问题描述主体层.save_pretrained("./第25轮保存/问题描述主体层/")
            模型.Input_description_layer.save_pretrained("./第25轮保存/Input_description_layer/")
            模型.Output描述层.save_pretrained("./第25轮保存/Output描述层/")
            模型.输入输出样例测试和Note描述层.save_pretrained("./第25轮保存/输入输出样例测试和Note描述层/")
        ...
        # ----------------------------------------------- 写入日志 ----------------------------------------------------#
        日志.info(f'\033[0:34m===== 训练集,第 {某轮+1} 轮结束，平均损失：{mean(训练损失列表)}。模型已经保存。总运行时间：{结束时间 - 开始时间} =======\033[m')
...
def 预测函数(模型, 设备, 测试集数据, 多块GPU, 命令行参数):
    ...
    with torch.no_grad():
        ...
        # -------------------------------- 预测结果：写入日志 -------------------------------------------------#
        日志.info(f'\033[0:32m预测模式:  损失： {mean(损失列表):.3f}\033[m')
