...
def setup_train_args():
    ...
    parser.add_argument('--topk', default=3, type=int, required=False, help='最高k选1，默认8')
    parser.add_argument('--topp', default=0.7, type=float, required=False, help='最高积累概率，默认0.9')  # 0
    parser.add_argument('--temperature', default=0.25, type=float, required=False, help='温度')  # 0
...
def complementary_fill_function(A_batch_data):
    ...
    """
    # ---------------------------------------------------------- 转化为张量字典 --------------------------------#
    General_question_text张量字典 = Tokenized_Bert_word_list(A_batch_data[0][0], max_length=2048, truncation=True, padding=True, return_tensors="pt")
    certain_slow_code_dictionary = Tokenized(A_batch_data[0][1], max_length=768, truncation=True, padding=True, return_tensors="pt")

    return [General_question_text张量字典, certain_slow_code_dictionary], A_batch_data[0][2], A_batch_data[0][3]
...
def Prediction_generation_function(model, devices, Dataset, Multiple_GPUs, args, Training_set_or_test_set):
    ...
    with torch.no_grad():
        ...
        for Batch_index, (Feature_List, label_code, Data_original_path) in enumerate(dataloader):
            ...
            # -------------------------------- 计算Bleu分数 --------------------------------------------#
            Standard_answer_list = [  # First set of references
                [label_code],
