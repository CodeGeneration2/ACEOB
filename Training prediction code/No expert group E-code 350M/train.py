...
def complementary_fill_function(A_batch_data):
    ...
    for a_data in A_batch_data:
        General_question_text.append(a_data[0])
        certain_slow_code.append(a_data[1])
        label_code.append(a_data[2])

    # ---------------------------------------------------------- 转化为张量字典 --------------------------------#
    General_question_text张量字典 = tokenBertVocabulary(General_question_text, max_length=2048, truncation=True, padding=True, return_tensors="pt")
    ...
    return [General_question_text张量字典, certain_slow_code_dictionary, label_code_dictionary]


# ##################################################################### 训练函数 #######################################
def train_Functions(model, devices, Training_set_data, test_code数据, Multiple_GPUs, args):
    ...
    for epoch in range(args.epochs):
        ...
        # ======================================================== 每结束一轮，保存model一次 ============================#
        model.Opened_expert_group_layer.save_pretrained(f"./{args.Generated_models}/Opened_expert_group_layer/")
        ...
        # ----------------------------------------------- 写入logger ----------------------------------------------------#
        logger.info(f'\033[0:34m===== 训练集,第 {epoch+1} 轮结束，平均loss：{mean(Training_loss_list)}。model已经保存 =======\033[m')

        # ------------------------------- 测试model ------------------#
        test_average_loss = predictive_Functions(model, devices, test_code数据, Multiple_GPUs, args)
...
def predictive_Functions(model, devices, test_code数据, Multiple_GPUs, args):
    # -------------------------------------- 开始预测：写入logger -----------------------------#
    ...
    test_dataloader = DataLoader(test_code数据,  # 使用的Dataset
                         batch_size=args.batch_size,  # 批处理样本大小
                         shuffle=True,  # 每次迭代前打乱数据
                         num_workers=args.num_workers,  # 使用一个进程
                         collate_fn=complementary_fill_function
