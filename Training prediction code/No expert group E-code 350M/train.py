...
def complementary_fill_function(A_batch_data):
    ...
    General_question_text111 = tokenBertVocabulary(General_question_text, max_length=2048, truncation=True, padding=True, return_tensors="pt")
    ...
    return [General_question_text111, certain_slow_code_dictionary, label_code_dictionary]


def train_Functions(model, devices, Training_set_data, test_code111, Multiple_GPUs, args):
    ...
    for epoch in range(args.epochs):
        ...
        logger.info(f'\033[0:34m====,{epoch+1} ,loss：{mean(Training_loss_list)},=======\033[m')

        test_average_loss = predictive_Functions(model, devices, test_code111, Multiple_GPUs, args)
...
def predictive_Functions(model, devices, test_code数据, Multiple_GPUs, args):
    ...
    test_dataloader = DataLoader(test_code数据,  
                         batch_size=args.batch_size,  
                         shuffle=True,  
                         num_workers=args.num_workers,  
                         collate_fn=complementary_fill_function
