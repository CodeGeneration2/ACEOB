...
class 测试集Dataset类(torch.utils.data.Dataset):
    def __init__(self, 模型="EleutherAI/gpt-neo-125M", 最大Token数=2046, 数据集根路径="../../数据集/test"):
    ...
    def 从当前目录初始化函数(self, 数据集根路径):
        ...
        #for 某条 in tqdm(range(1,2)):
            ...
            # ------------------------------------------- 标题 ---------------------------------#
            with open(f"{数据集根路径}/{某条}/question.txt", 'r', encoding='UTF-8') as f:
                总问题文本 = f.read()
            # # ------------------------------------------- 标题 ---------------------------------#
            ...
            for 某代码 in 慢速代码集列表:
                ...
                if len(某慢速代码张量字典["input_ids"][0]) > 768:
                    print(f'\033[0:35m删选，过大的， 第 {某条} 条的长度为: {len(某慢速代码张量字典["input_ids"][0])}。标签代码张量字典["input_ids"]>2048 033[m')
                    continue

                # ---------------------------------------- 数据原路径 ----------------------------------------#
                数据原路径 = f"{数据集根路径}/{某条}/acc_tle_solutions/{某代码}"
                # ---------------------------------------- 最小单位 ----------------------------------------#
                某条数据元组 = (总问题文本, 某慢速代码, 标签代码, 数据原路径)
