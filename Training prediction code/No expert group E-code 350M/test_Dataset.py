...
class Test_set_Dataset_class(torch.utils.data.Dataset):
    ...
    def initialize_function_from_current_directory(self, dataset_root_path):
        ...
        #for i in tqdm(range(1,2)):
            ...
            # ------------------------------------------- title ---------------------------------#
            with open(f"{dataset_root_path}/{i}/question.txt", 'r', encoding='UTF-8') as f:
                General_question_text = f.read()
            ...
            for a_code in Slow_code_set_list:
                ...
                if len(certain_slow_code_dictionary["input_ids"][0]) > 768:
                    # print(f'\033[0:35m删选，过大的， 第 {i} 条的长度为: {len(certain_slow_code_dictionary["input_ids"][0])}。label_code_dictionary["input_ids"]>2048 033[m')
                    continue

                # ---------------------------------------- Data_original_path ----------------------------------------#
                Data_original_path = f"{dataset_root_path}/{i}/acc_tle_solutions/{a_code}"
                # ---------------------------------------- 最小单位 ----------------------------------------#
                A_data_tuple = (General_question_text, certain_slow_code, label_code, Data_original_path)
