...
def 接收命令行参数函数():
    ...
    命令行解析器.add_argument('--topk', default=4, type=int, required=False, help='最高k选1，默认8')
    命令行解析器.add_argument('--topp', default=0.8, type=float, required=False, help='最高积累概率，默认0.9')  # 0
    命令行解析器.add_argument('--temperature', default=0.25, type=float, required=False, help='最高积累概率，默认0.9')  # 0
...
def 补足填充函数(一批数据):
    ...
    """
    # ---------------------------------------------------------- 转化为张量字典 --------------------------------#
    问题文本张量字典 = Token化Bert词表(一批数据[0][0], max_length=512, truncation=True, padding=True, return_tensors="pt")
    某慢速代码张量字典 = Token化词表(一批数据[0][1], max_length=768, truncation=True, padding=True, return_tensors="pt")

    新问题文本张量 = torch.cat((问题文本张量字典["input_ids"], 某慢速代码张量字典["input_ids"]), dim=1)
    新问题注意力张量 = torch.cat((问题文本张量字典["attention_mask"], 某慢速代码张量字典["attention_mask"]), dim=1)

    新问题文本张量字典 = {"input_ids":新问题文本张量,"attention_mask":新问题注意力张量}


    return [新问题文本张量字典, 某慢速代码张量字典], 一批数据[0][2], 一批数据[0][3]
