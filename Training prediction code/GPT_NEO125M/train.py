...
Token化Bert词表 = BertTokenizer.from_pretrained("Bert词表/", pad_token="[PAD]")
...
def 补足填充函数(一批数据):
    ...
    # ---------------------------------------------------------- 转化为张量字典 --------------------------------#
    问题文本张量字典 = Token化词表(问题文本, max_length=512, truncation=True, padding=True, return_tensors="pt")
    ...
    # ---------------------------------------------------- 特殊 ---------------------------------#
    新问题文本张量 = torch.cat((问题文本张量字典["input_ids"], 某慢速代码张量字典["input_ids"], 标签代码张量字典["input_ids"]), dim=1)
    新问题注意力张量 = torch.cat((问题文本张量字典["attention_mask"], 某慢速代码张量字典["attention_mask"], 标签代码张量字典["attention_mask"]), dim=1)

    新问题文本张量字典 = {"input_ids":新问题文本张量,"attention_mask":新问题注意力张量}


    新标签张量列表 = [[]]
    新标签张量列表[0].extend([-100] * len(问题文本张量字典["input_ids"][0]))
    新标签张量列表[0].extend([-100] * len(某慢速代码张量字典["input_ids"][0]))

    新标签张量 = torch.LongTensor(新标签张量列表)
    新标签张量 = torch.cat((新标签张量, 标签代码张量字典["input_ids"]), dim=1)

    新标签张量字典 = {"input_ids":新标签张量,"attention_mask":新标签张量}


    return [新问题文本张量字典, 某慢速代码张量字典, 新标签张量字典]
