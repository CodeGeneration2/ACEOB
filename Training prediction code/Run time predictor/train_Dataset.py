...
class 训练集Dataset类(torch.utils.data.Dataset):
    ...
    def 从当前目录初始化函数(self, 数据集根路径):
        ...
        #for 某条 in tqdm(range(4)):
            ...
            # ----------------------------------------------- 标签 --------------------------------#
            with open(f"{数据集根路径}/{某条}/accepted run time.txt", 'r', encoding='UTF-8') as f:
                运行时间 = f.read().split("accepted,")[1].split(" ms,")[0]
            ...
            for 某代码 in 慢速代码集列表:
                ...
                # ---------------------------------------- 最小单位 ----------------------------------------#
                运行时间 = int(某代码.split(",")[1].split(" ms")[0])
                某条数据元组 = (某慢速代码, 运行时间)
            ...
            # ============================================================= 特征 ====================================#
            较优代码集列表 = os.listdir(f"{数据集根路径}/{某条}/acc_solutions")

            # ============================================== 导入 某条 特征集 ===================================#
            for 某代码 in 较优代码集列表:
                with open(f"{数据集根路径}/{某条}/acc_solutions/{某代码}", 'r', encoding='UTF-8') as f:
                    某较优代码 = f.read()

                # ------------------------------- 缩进代码 ----------------------#
                某较优代码 = 缩进代码函数(某较优代码)
                某较优代码张量字典 = self.Token化词表(某较优代码, return_tensors="pt")
                if len(某较优代码张量字典["input_ids"][0]) > 768:
                    print(f'\033[0:34m删选， 过大的， 第 {某条} 条的长度为: {len(某较优代码张量字典["input_ids"][0])}。某慢速代码张量字典["input_ids"]>2048 033[m')
                    continue
