...
def 接收命令行参数函数():
    ...
    命令行解析器.add_argument('--lr', default=1.5e-3, type=float, required=False, help='学习率')
...
def 补足填充函数(一批数据):
    ...
    # ---------------------------------------------------------- 转化为张量字典 --------------------------------#
    代码张量字典 = Token化词表(代码, max_length=768, truncation=True, padding=True, return_tensors="pt")

    总运行时间.append(运行时间)
    运行时间张量 = torch.Tensor(总运行时间)


    return [代码张量字典, 运行时间张量]
...
def train函数(模型, 设备, 训练集数据, 测试集数据, 多块GPU, 命令行参数):
    ...
    for 某轮 in range(命令行参数.epochs):
        ...
        for 批量索引, [代码张量字典, 运行时间张量] in enumerate(训练数据加载器):
        ...
        # ======================================================== 每结束一轮，保存模型一次 ============================#
        torch.save(模型.全连接层, "./已生成模型/全连接层.pkl")
        模型.代码层.save_pretrained("./已生成模型/代码层/")

        # ======================================================== 奇数轮备用：间隔保存，安全保障 ==================#
        if (某轮 + 1) % 2 == 1:
            torch.save(模型.全连接层, "./奇数轮备用/全连接层.pkl")
            模型.代码层.save_pretrained("./奇数轮备用/代码层/")

        # ======================================================== 20备用：间隔保存，安全保障 ==================#
        if (某轮 + 1)  == 20:
            torch.save(模型.全连接层, "./第20轮保存/全连接层.pkl")
            模型.代码层.save_pretrained("./第20轮保存/代码层/")

        # ======================================================== 25备用：间隔保存，安全保障 ==================#
        if (某轮 + 1) == 25:
            torch.save(模型.全连接层, "./第25轮保存/全连接层.pkl")
            模型.代码层.save_pretrained("./第25轮保存/代码层/")
...
def 预测函数(模型, 设备, 测试集数据, 多块GPU, 命令行参数):
    ...
    with torch.no_grad():
        损失列表 = []
        for 批量索引, [代码张量字典, 运行时间张量] in enumerate(测试数据加载器):
